...
func (h *CubeHandler) SpendCubeTicket(w http.ResponseWriter, r *http.Request) {
	username := auth.GetUsername(r)
	var req struct {
	...
	if err := h.svc.SpendCubeTicket(r.Context(), username, req.CharacterID); err != nil {
		writeError(w, http.StatusBadRequest, err.Error())
		return
	}
	writeJSON(w, http.StatusOK, map[string]string{"message": "cube ticket spent"})
}
