...
func NewCharacterDayHandler(ds *service.CharacterDayService, cs *service.CharacterService) *CharacterDayHandler {
	return &CharacterDayHandler{dayService: ds, charService: cs}
}
...
func (h *CharacterDayHandler) CheckDayContent(w http.ResponseWriter, r *http.Request) {
	...
	_, err := h.dayService.CheckDayContent(r.Context(), username, req)
	if err != nil {
		writeError(w, http.StatusBadRequest, err.Error())
		return
	}

	// Return full CharacterResponse (matching Spring)
	resp, err := h.charService.GetCharacterByID(r.Context(), req.CharacterID)
	if err != nil {
		writeError(w, http.StatusBadRequest, err.Error())
		return
	}
...
func (h *CharacterDayHandler) CheckAllDayContent(w http.ResponseWriter, r *http.Request) {
	...
	_, err := h.dayService.CheckAllDayContent(r.Context(), username, req)
	if err != nil {
		writeError(w, http.StatusBadRequest, err.Error())
		return
	}
