...
// DeleteStatus handles DELETE /api/v1/general-todos/status/{statusId}
...
// CreateCategoryForFolder handles POST /api/v1/general-todos/categories/folders/{folderId}
// Extracts folderId from URL path param (Spring Boot @PathVariable pattern).
func (h *GeneralTodoHandler) CreateCategoryForFolder(w http.ResponseWriter, r *http.Request) {
	username := auth.GetUsername(r)
	folderID := parseInt64Param(getPathParam(r, "folderId"))
	var req service.CreateCategoryRequest
	if err := readJSON(r, &req); err != nil {
...
// CreateStatusForCategory handles POST /api/v1/general-todos/categories/{categoryId}/statuses
func (h *GeneralTodoHandler) CreateStatusForCategory(w http.ResponseWriter, r *http.Request) {
	username := auth.GetUsername(r)
	categoryID := parseInt64Param(getPathParam(r, "categoryId"))
	var req service.CreateStatusRequest
	if err := readJSON(r, &req); err != nil {
	...
	req.ItemID = categoryID // categoryId maps to itemId in service layer
	status, err := h.svc.CreateStatus(r.Context(), username, req)
	if err != nil {
		writeError(w, http.StatusBadRequest, err.Error())
		return
...
// ToggleStatusInCategory handles PATCH /api/v1/general-todos/categories/{categoryId}/statuses/{statusId}
func (h *GeneralTodoHandler) ToggleStatusInCategory(w http.ResponseWriter, r *http.Request) {
	username := auth.GetUsername(r)
	statusID := parseInt64Param(getPathParam(r, "statusId"))
	if err := h.svc.ToggleStatus(r.Context(), username, statusID); err != nil {
		writeError(w, http.StatusBadRequest, err.Error())
...
// DeleteStatusInCategory handles DELETE /api/v1/general-todos/categories/{categoryId}/statuses/{statusId}
func (h *GeneralTodoHandler) DeleteStatusInCategory(w http.ResponseWriter, r *http.Request) {
	username := auth.GetUsername(r)
	statusID := parseInt64Param(getPathParam(r, "statusId"))
	if err := h.svc.DeleteStatus(r.Context(), username, statusID); err != nil {
		writeError(w, http.StatusBadRequest, err.Error())
