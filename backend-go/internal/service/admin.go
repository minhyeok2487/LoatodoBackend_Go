...
func (s *AdminService) GetDashboard(ctx context.Context) (*DashboardResponse, error) {
	...
	err := s.db.QueryRowContext(ctx, `
		SELECT
			...
			(SELECT COUNT(*) FROM characters WHERE is_deleted = false) as total_characters,
...
func (s *AdminService) ListMembers(ctx context.Context, search string, page, size int) (*AdminMemberListResponse, error) {
	...
	selectQuery := fmt.Sprintf(
		"SELECT member_id, username, role, created_date FROM member %s ORDER BY created_date DESC LIMIT ? OFFSET ?",
		where,
	)
...
func (s *AdminService) GetMemberDetail(ctx context.Context, memberID int64) (*AdminMemberDetailResponse, error) {
	...
	err := s.db.QueryRowContext(ctx,
		"SELECT member_id, username, role, created_date FROM member WHERE member_id = ?",
		memberID,
	...
	rows, err := s.db.QueryContext(ctx,
		`SELECT characters_id, character_name, server_name, item_level, character_class_name
		 FROM characters WHERE member_id = ? AND is_deleted = false ORDER BY sort_number ASC`,
		memberID,
	)
	if err != nil {
...
func (s *AdminService) UpdateMember(ctx context.Context, memberID int64, req UpdateMemberRequest) error {
	...
	result, err := s.db.ExecContext(ctx,
		"UPDATE member SET role = ?, last_modified_date = ? WHERE member_id = ?",
...
func (s *AdminService) DeleteMember(ctx context.Context, memberID int64) error {
	...
	_, err = tx.ExecContext(ctx, "DELETE FROM notification WHERE member_id = ?", memberID)
	if err != nil {
	...
	_, err = tx.ExecContext(ctx,
		"DELETE FROM community_images WHERE community_id IN (SELECT community_id FROM community WHERE member_id = ?)",
		memberID,
	)
	if err != nil {
	...
	_, err = tx.ExecContext(ctx, "DELETE FROM characters WHERE member_id = ?", memberID)
	if err != nil {
		return fmt.Errorf("캐릭터 삭제 실패: %w", err)
	}

	// Delete the member
	result, err := tx.ExecContext(ctx, "DELETE FROM member WHERE member_id = ?", memberID)
	if err != nil {
...
func (s *AdminService) ListCharacters(ctx context.Context, page, size int) (*AdminCharacterListResponse, error) {
	// Count total
	var totalCount int64
	err := s.db.QueryRowContext(ctx, "SELECT COUNT(*) FROM characters WHERE is_deleted = false").Scan(&totalCount)
	if err != nil {
	...
	offset := page * size
	rows, err := s.db.QueryContext(ctx,
		`SELECT c.characters_id, c.character_name, c.server_name, c.item_level, c.character_class_name, m.username
		 FROM characters c
...
func (s *AdminService) ListAllComments(ctx context.Context) ([]AdminCommentResponse, error) {
	rows, err := s.db.QueryContext(ctx,
		`SELECT c.comments_id, c.community_id, m.username, c.body, c.created_date
		 FROM comments c
		 JOIN member m ON c.member_id = m.member_id
...
func (s *AdminService) DeleteComment(ctx context.Context, commentID int64) error {
	result, err := s.db.ExecContext(ctx, "DELETE FROM comments WHERE comments_id = ?", commentID)
	if err != nil {
		return fmt.Errorf("댓글 삭제 실패: %w", err)
	}
...
func (s *AdminService) ListAllNotifications(ctx context.Context) ([]AdminNotificationResponse, error) {
	rows, err := s.db.QueryContext(ctx,
		`SELECT n.notification_id, m.username, n.content, n.is_read, n.created_date
		 FROM notification n
...
func (s *AdminService) CreateNotification(ctx context.Context, req CreateNotificationRequest) error {
	...
	err := s.db.QueryRowContext(ctx, "SELECT member_id FROM member WHERE username = ?", req.Username).Scan(&receiverID)
	if err != nil {
		if errors.Is(err, sql.ErrNoRows) {
	...
	_, err = s.db.ExecContext(ctx,
		`INSERT INTO notification (content, is_read, notification_type, member_id, created_date, last_modified_date)
		 VALUES (?, false, 'ADMIN', ?, ?, ?)`,
...
func (s *AdminService) ListContent(ctx context.Context) ([]AdminContentResponse, error) {
	rows, err := s.db.QueryContext(ctx,
		"SELECT content_id, name, category, enabled FROM content ORDER BY content_id ASC",
	)
	if err != nil {
...
func (s *AdminService) UpdateContent(ctx context.Context, contentID int64, req UpdateContentRequest) error {
	result, err := s.db.ExecContext(ctx,
		"UPDATE content SET name = ?, category = ?, enabled = ? WHERE content_id = ?",
...
func (s *AdminService) DeleteCharacter(ctx context.Context, characterID int64) error {
	_, err := s.db.ExecContext(ctx, "DELETE FROM characters WHERE characters_id = ?", characterID)
	return err
}

func (s *AdminService) DeleteFriendByAdmin(ctx context.Context, friendID int64) error {
	_, err := s.db.ExecContext(ctx, "DELETE FROM friends WHERE friends_id = ?", friendID)
	return err
}

func (s *AdminService) DeleteNotificationByAdmin(ctx context.Context, notificationID int64) error {
	_, err := s.db.ExecContext(ctx, "DELETE FROM notification WHERE notification_id = ?", notificationID)
	return err
}

func (s *AdminService) GetContentDetail(ctx context.Context, contentID int64) (map[string]interface{}, error) {
	var id int64
	var dtype, name, category string
	var level float64
	err := s.db.QueryRowContext(ctx,
		"SELECT content_id, dtype, name, category, level FROM content WHERE content_id = ?", contentID,
	).Scan(&id, &dtype, &name, &category, &level)
	if err != nil {
		return nil, fmt.Errorf("content not found: %w", err)
	}
	return map[string]interface{}{"id": id, "dtype": dtype, "name": name, "category": category, "level": level}, nil
}

func (s *AdminService) CreateContent(ctx context.Context, req map[string]interface{}) (map[string]interface{}, error) {
	dtype, _ := req["dtype"].(string)
	name, _ := req["name"].(string)
	category, _ := req["category"].(string)
	level, _ := req["level"].(float64)
	now := time.Now()
	result, err := s.db.ExecContext(ctx,
		"INSERT INTO content (dtype, name, category, level, created_date, last_modified_date) VALUES (?, ?, ?, ?, ?, ?)",
		dtype, name, category, level, now, now,
	)
	if err != nil {
		return nil, fmt.Errorf("inserting content: %w", err)
	}
	id, _ := result.LastInsertId()
	return map[string]interface{}{"id": id, "dtype": dtype, "name": name, "category": category, "level": level}, nil
}

func (s *AdminService) DeleteContent(ctx context.Context, contentID int64) error {
	_, err := s.db.ExecContext(ctx, "DELETE FROM content WHERE content_id = ?", contentID)
	return err
}
