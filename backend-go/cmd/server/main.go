...
func main() {
	...
	emailService := service.NewEmailService(db, cfg.SMTPHost, cfg.SMTPPort, cfg.SMTPEmail, cfg.SMTPPassword)
	...
	characterDayHandler := handler.NewCharacterDayHandler(characterDayService, characterService)
	characterWeekHandler := handler.NewCharacterWeekHandler(characterWeekService, characterService)
	...
	rateLimiter := middleware.NewRateLimiter(200, 500)
