...
func (s *CubeService) GetCubeData(ctx context.Context, username string) ([]CubeResponse, error) {
	...
	rows, err := s.db.QueryContext(ctx,
		`SELECT cu.id, c.character_name, cu.ban1, cu.ban2, cu.ban3
		 FROM cubes cu
		 JOIN character_entity c ON cu.character_id = c.id
...
func (s *CubeService) CreateOrUpdateCube(ctx context.Context, username string, req CreateCubeRequest) (*CubeResponse, error) {
	...
	err = s.db.QueryRowContext(ctx,
		`SELECT id FROM character_entity
		 WHERE character_name = ? AND member_id = ? AND (deleted IS NULL OR deleted = false)`,
	...
	err = s.db.QueryRowContext(ctx,
		`SELECT id FROM cubes WHERE character_id = ?`, characterID).Scan(&existingID)
...
func (s *CubeService) DeleteCube(ctx context.Context, username string, cubeID int64) error {
	...
	result, err := s.db.ExecContext(ctx,
		`DELETE FROM cubes WHERE id = ? AND character_id IN (
			SELECT id FROM character_entity WHERE member_id = ?
		)`, cubeID, memberID)
	if err != nil {
...
func (s *CubeService) UpdateCube(ctx context.Context, username string, req UpdateCubeRequest) error {
	...
	err = s.db.QueryRowContext(ctx,
		`SELECT c.member_id FROM cubes cu
		 JOIN character_entity c ON cu.character_id = c.id
		 WHERE cu.id = ?`, req.CubeID).Scan(&ownerID)
	if err != nil {
