...
func (s *FriendService) GetFriends(ctx context.Context, username string) ([]FriendsResponse, error) {
	...
	query := `
		SELECT f.id, m2.username, f.are_we_friend, f.ordering,
...
func (s *FriendService) SearchCharacter(ctx context.Context, username, characterName string) ([]FriendFindCharacterResponse, error) {
	...
	// 2. Query characters by name, excluding requesting user
	query := `
		SELECT DISTINCT m.id, m.username, c.character_name,
		       (SELECT COUNT(*) FROM character_entity WHERE member_id = m.id AND deleted = false) as char_count
		FROM character_entity c
		JOIN member m ON c.member_id = m.id
		WHERE c.character_name LIKE ? AND m.username != ?
	`
	...
	for rows.Next() {
		...
		if err != nil {
			return nil, fmt.Errorf("친구 상태 확인 실패: %w", err)
		}

		if friendCount > 0 {
			// Check if both sides accepted
			var myAreWeFriend, theirAreWeFriend bool
			...
			switch {
			case myAreWeFriend && theirAreWeFriend:
				resp.AreWeFriend = "깐부"
			case myAreWeFriend && !theirAreWeFriend:
...
func (s *FriendService) SendFriendRequest(ctx context.Context, username string, req FriendRequest) error {
	...
	err := s.db.QueryRowContext(ctx, "SELECT id FROM member WHERE username = ?", username).Scan(&toMemberID)
	if err != nil {
	...
	var charCount int
	err = s.db.QueryRowContext(ctx,
		"SELECT COUNT(*) FROM character_entity WHERE member_id = ? AND deleted = false",
		toMemberID,
	).Scan(&charCount)
	if err != nil {
	...
	err = s.db.QueryRowContext(ctx, "SELECT id FROM member WHERE username = ?", req.FriendUsername).Scan(&fromMemberID)
	if err != nil {
	...
	_, err = tx.ExecContext(ctx,
		`INSERT INTO notification (content, is_read, notification_type, receiver_id, friend_username, created_date, last_modified_date)
...
func (s *FriendService) HandleFriendRequest(ctx context.Context, username string, req FriendRequestAction) error {
	...
	err = s.db.QueryRowContext(ctx, "SELECT id FROM member WHERE username = ?", req.FriendUsername).Scan(&friendMemberID)
	if err != nil {
		return fmt.Errorf("대상 회원을 찾을 수 없습니다: %w", err)
	}
...
func (s *FriendService) UpdateSettings(ctx context.Context, username string, req FriendSettingsUpdateRequest) (*FriendSettings, error) {
	...
	err := s.db.QueryRowContext(ctx,
		"SELECT m.username FROM friends f JOIN member m ON f.member_id = m.id WHERE f.id = ?",
		req.ID,
	).Scan(&ownerUsername)
	if err != nil {
	...
	query := fmt.Sprintf("UPDATE friends SET %s = ?, last_modified_date = ? WHERE id = ?", column)
...
func (s *FriendService) DeleteFriend(ctx context.Context, username string, friendID int64) error {
	...
	err = s.db.QueryRowContext(ctx,
		"SELECT from_member FROM friends WHERE id = ? AND member_id = ?",
		friendID, memberID,
	).Scan(&fromMember)
	if err != nil {
...
func (s *FriendService) UpdateSortOrder(ctx context.Context, username string, req FriendSortRequest) error {
	...
	for i, friendID := range req.FriendIDList {
		result, err := tx.ExecContext(ctx,
			"UPDATE friends SET ordering = ?, last_modified_date = ? WHERE id = ? AND member_id = ?",
